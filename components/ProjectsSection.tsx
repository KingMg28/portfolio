      ...
      <div className="projects-list grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {projects.map(
          ({ id, image, description, title, gitHubLink, link, tags }) => (
            <div
              key={id}
              className="project-item relative  w-fit h-fit  bg-neutral-900  border border-neutral-700 p-4 rounded-xl shadow-lg hover:shadow-pink-800 hover:scale-105 transition-all duration-500"
            >
              <Image
                src={image}
                alt={title}
              ...
              />
              <div className="absolute inset-0 flex items-center justify-center gap-2 text-black text-3xl opacity-0 hover:opacity-100 transition-opacity duration-300">
